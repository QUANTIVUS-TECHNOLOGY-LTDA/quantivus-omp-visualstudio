...
namespace VSAgent.Commands
{
    internal sealed class ExplainCodeCommand
    {
        ...
        private ExplainCodeCommand(AsyncPackage package, OleMenuCommandService commandService, DTE2 dte)
        {
        ...
        public static async Task InitializeAsync(AsyncPackage package)
        {
            ...
            Instance = new ExplainCodeCommand(package, commandService, dte);
        }
        ...
        private async void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(code)) code = editorContext.GetCurrentLine();
                if (string.IsNullOrWhiteSpace(code)) { ShowError("Select code or place the caret on a source line first."); return; }
                var result = await agentService.ExplainCodeAsync(code);
                await package.JoinableTaskFactory.SwitchToMainThreadAsync();
                ShowResult("Code Explanation", result);
            }
            catch (Exception ex)
            {
                await package.JoinableTaskFactory.SwitchToMainThreadAsync();
                ShowError("Error explaining code: " + ex.Message);
            }
        }
